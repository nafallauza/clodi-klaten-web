    ...
    <article class="product-card flex flex-col h-full bg-white rounded-2xl shadow-md overflow-hidden">
      ...
      <div class="product-info p-5 flex flex-col flex-grow">
        ...
        <div class="product-price mt-4 flex items-end justify-between">
          <div>
            ${product.original_price ? `<span class="price-original line-through text-gray-400 text-sm">Rp ${Number(product.original_price).toLocaleString("id-ID")}</span>` : ""}
            <span class="price-current text-sky-600 text-xl font-bold block">${formatPrice(product.price)}</span>
          </div>
        </div>
        <!-- Tombol WhatsApp -->
        <a href="https://api.whatsapp.com/send?phone=${waNumber}&text=${waMessage}" target="_blank" class="flex items-center justify-center gap-2 w-full bg-[#25D366] hover:bg-[#1ebe57] text-white font-medium py-2.5 rounded-lg transition duration-300 mt-auto text-[15px]">
